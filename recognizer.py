...
maskList = ["iVBORw0KGgoAAAANSUhEUgAAABUAAAAyCAYAAACzvpAYAAAAdklEQVR4nO3UQQ6AIAxEUfD+dx43Soixdlp10TB/a/pSIqE1tW49OgAAY7j32/ntzUZWIXTe8hOUBUNoJAo9t7R+TAqdQQZ2UQBgN6TQDOii2R7RzJYumk1oEZS6Mt4Ldb16dY7/C5oKR9b3OscXWgRVSqkF2wFgkSQzrtPKgQAAAABJRU5ErkJggg==",
...
def recognize(img_path):
    ...
    captcha = ""
    global maskList
    originalimage = Image.open(open(img_path, 'rb'))
    processedimage = originalimage.convert('L').point(
        lambda x: 255 if x > 20 else 0, mode='1').convert('1').convert('RGBA')
        ...
        for i in range(15, 120): # leftmost mask position, rigthmost mask position
            ...
            if resultimage.getextrema() == ((0, 0), (0, 0), (0, 0), (255, 255)):
                if z == 4: # fill 4 with white to exclude false 1 on 4
                    ...
                    processedimage.paste(Image.alpha_composite(
                        processedimage.crop((i, 0, i + 21, 0 + 50)), maskx), (i, 0))
                if previ == 0 or i > previ + 15: #no digit closer then 15 px
                    Zlist.append((i, z))
                    if len(Zlist) == 5:
                        Zlist.sort()
                        for z in Zlist:  # !! redefining name from outer scope
